		...
		if (options?.client) {
			for (const function_ of methods) {
				if (typeof options.client[function_] !== 'function') {
					throw new TypeError(
						'An invalid memcached client was passed to store.',
					);
				}
			}
	...
	async increment(key: string): Promise<IncrementResponse> {
		...
		if (totalHits === false) {
			try {
				...
				// xo currently has conflicting rules around unnecessary parens - https://github.com/xojs/xo/issues/889
		...
		return {
			...
			resetTime:
				typeof expiresAt === 'number' ? new Date(expiresAt) : new Date(),
		};
	}
